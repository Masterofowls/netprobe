import type { AppSettings, NetProbeBackup, Resource } from "../types";
import { isOfflineStatus } from "./notificationService";
...
export const buildStatusShareText = (resources: Resource[]): string => {
  ...
  lines.push("", "https://netprobe-1.vercel.app");
  return lines.join("\n");
};

export const downloadBackupOnWeb = (backup: NetProbeBackup): void => {
  ...
  anchor.href = url;
  anchor.download = `netprobe-backup-${new Date().toISOString().slice(0, 10)}.json`;
  anchor.click();
